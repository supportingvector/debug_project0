...
class SocialDataLoader():
    ...
    def __init__(self, batch_size=50, seq_length=8, maxNumPeds=40, datasets=[0, 1, 2, 3, 4], forcePreProcess=False):
        '''
        ...
        # self.used_data_dirs=[]
        # for x in datasets:
            # self.used_data_dirs.append(self.data_dirs[x])
    ...
    def frame_preprocess(self, data_dirs, data_file):
        ...
        for directory in data_dirs:
            ...
            for frame in frameList:
                ...
                #data[0,:]==frame，返回对每一列第一行为frame的布尔值
                #data[:, data[0, :] == frame]提取布尔值为真的列
                # Extract peds list
                #把这一帧的行人id存下来，为pedsList
                ...
                #all_frame_data[第几个dataset][frame_id][0到这个frame的ped个数][三维]=pedwithpos([ped个数][3维度])
    ...
    def next_batch(self):
        ...
        while i < self.batch_size:
            ...
            if idx + self.seq_length < frame_data.shape[0]:
                ...
                #从frame [idx到idx+seq_length+1]所有data（[0到这个frame的ped个数][三维]）（三维指ped行人id,x,y）
                seq_source_frame_data = frame_data[idx:idx+self.seq_length, :]
                #[idx到idx+seq_length]作为RNN模型输入
                seq_target_frame_data = frame_data[idx+1:idx+self.seq_length+1, :]
                #[idx+1到idx+seq_length+1]作为RNN预测
                # Number of unique peds in this sequence of frames
                pedID_list = np.unique(seq_frame_data[:, :, 0])
                #把idx到idx+seq_length+1这个时间段所有的ped的id号列出来作为list
                numUniquePeds = pedID_list.shape[0]
                #记录ID号
                sourceData = np.zeros((self.seq_length, self.maxNumPeds, 3))
                #列出一个seq_length中所有Ped,(pedid和对应的x,y)
                ...
                for seq in range(self.seq_length):
                    ...
                    #当前frame的所有source data（[0到这个frame的ped个数][三维]）（三维指ped行人id,x,y）
                    tseq_frame_data = seq_target_frame_data[seq, :]
                    #target Data，即source data的下一帧
                    for ped in range(numUniquePeds):
                        ...
                        if pedID == 0:
                            ...
                            #提出特定帧特定ped的data(PEDID,x,y)
                            tped = np.squeeze(tseq_frame_data[tseq_frame_data[:, 0] == pedID, :])
                            #提出特定帧下一帧预测的ped的data(PEDID,x,y)
                            if sped.size != 0:
                                sourceData[seq, ped, :] = sped
                                #第seq个时间点的第ped个行人的数据(PEDID,x,y)
                            if tped.size != 0:
